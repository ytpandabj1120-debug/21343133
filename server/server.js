...
function buildStateForPlayer(room, viewerId) {
  ...
  const state = {
    ...
    players: room.players.map((player) => {
      const gameState = room.game?.playerStates?.[player.id] || null;
...
function calculatePayoutsAndHistory(room, timestamp) {
  ...
  // 구사 재경기에서 넘어온 기존 판돈은 이번 재경기 승자에게 별도로 지급합니다.
  // v4에서는 이 carry-over 금액이 game.pot에는 들어갔지만 totalContribution에는 없어서
  // 정산에서 빠지는 문제가 있었습니다. 여기서 반드시 승자에게 배분합니다.
  const deadPot = Number(game.deadPot || 0);
  if (deadPot > 0) {
    const eligibleIds = game.turnOrder.filter((playerId) => !game.playerStates[playerId].folded);
    if (eligibleIds.length > 0) {
      const winnerEntries = eligibleIds.map((playerId) => ({
        playerId,
        hand: game.playerStates[playerId].hand,
      ...
      const winners = findWinners(winnerEntries).map((entry) => entry.playerId);
      const shareBase = Math.floor(deadPot / winners.length);
      let shareRemainder = deadPot % winners.length;
...
async function queueReplayRound(room, replayPlayerIds, reason) {
  ...
  const foldedPlayerIds = game.turnOrder.filter((playerId) => game.playerStates[playerId].folded);
  const baseJoinCount = Math.max(1, activePlayerIds.length);
  const joinFee = Math.max(1, Math.ceil(game.pot / baseJoinCount));
  ...
  // 구사 재경기는 판돈을 없애지 않고 pendingStart에 보관합니다.
  // 5초 카운트다운 동안 전 판에서 다이했던 사람은 joinReplay 버튼으로 재참가할 수 있습니다.
  room.pendingStart = {
    skipAnte: true,
    carryPot: game.pot,
    deadPot: game.pot,
  ...
  addRoomMessage(
    room,
    `${replayPlayerIds.join(', ')}님의 구사로 현재 판돈 ${game.pot}조각을 유지한 채 재경기 대기 상태가 되었습니다.`
  );

  if (foldedPlayerIds.length > 0) {
    addRoomMessage(room, `전 판에서 다이한 플레이어는 ${joinFee}조각을 내고 재경기에 참가할 수 있습니다.`);
  }

  if (room.players.length >= MIN_PLAYERS) {
    addRoomMessage(room, '5초 후 자동으로 재경기가 시작됩니다. 참가하지 않은 사람은 이번 재경기에서 베팅할 수 없습니다.');
...
async function showdown(room) {
  ...
  const replayPlayers = playerHands.filter((entry) => entry.hand.replay).map((entry) => entry.playerId);
  if (replayPlayers.length > 0) {
    await queueReplayRound(room, replayPlayers, '구사가 나와 판돈을 유지한 채 재경기합니다.');
    return;
  }
...
async function startGame(room, options = {}) {
  ...
  const carryPot = Number(options.carryPot || 0);
  const deadPot = Number(options.deadPot || carryPot || 0);
  const participantIds = Array.isArray(options.participantIds) && options.participantIds.length > 0
    ? options.participantIds.filter((id) => getPlayerFromRoom(room, id))
    : getRoomPlayerIds(room);

  if (participantIds.length < MIN_PLAYERS) {
    throw new Error('재경기 참가자가 2명 미만이라 게임을 시작할 수 없습니다.');
  }

  if (!skipAnte) {
    for (const playerId of participantIds) {
      const enough = await hasEnoughChips(playerId, BASE_ANTE);
      if (!enough) {
        throw new Error(`${playerId}님의 조각이 부족하여 게임을 시작할 수 없습니다.`);
      }
    }

    for (const playerId of participantIds) {
      await changePlayerPieces(room, playerId, -BASE_ANTE);
    }
  }
  ...
  const turnOrder = allOrder.filter((playerId) => participantIds.includes(playerId));
  ...
  for (const playerId of turnOrder) {
    ...
    playerStates[playerId] = {
      ...
      currentBet: 0,
      totalContribution: skipAnte ? 0 : BASE_ANTE,
      hand: null,
    };
  }

  const initialActionableIds = turnOrder.filter((playerId) => !playerStates[playerId].allIn);
  const initialPot = carryPot + (skipAnte ? 0 : participantIds.length * BASE_ANTE);
  ...
  if (skipAnte && carryPot > 0) {
    addRoomMessage(
      room,
      `재경기가 시작되었습니다. 이전 판돈 ${carryPot}조각을 그대로 이어받고, 참가자만 베팅할 수 있습니다.`
    );
  } else {
...
io.on('connection', (socket) => {
  ...
  socket.on('joinReplay', async () => {
    try {
      ...
      if (!room || !playerId || !room.pendingStart?.isReplay) {
        emitError(socket, '현재 참가 가능한 재경기가 없습니다.');
        return;
      }

      if (!(room.pendingStart.joinableIds || []).includes(playerId)) {
        emitError(socket, '전 판에서 다이한 플레이어만 재경기 참가 버튼을 사용할 수 있습니다.');
        return;
      }
      ...
      const player = getPlayerFromRoom(room, playerId);
      if (!player) {
        emitError(socket, '플레이어 정보를 찾을 수 없습니다.');
        return;
      }

      if (joinFee > player.pieces) {
        emitError(socket, `재경기 참가비 ${joinFee}조각이 부족합니다.`);
        return;
      }
      ...
      room.pendingStart.carryPot += joinFee;
      room.pendingStart.deadPot += joinFee;

      if (room.lastResult?.replay) {
        room.lastResult.carryOverPot = room.pendingStart.carryPot;
      ...
      console.error('joinReplay error:', error);
      emitError(socket, error.message || '재경기 참가 중 오류가 발생했습니다.');
    }
  });
