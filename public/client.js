...
function updateAutoRestartText(nextAutoStartAt) {
  ...
  autoRestartBanner.textContent = currentState?.pendingReplay
    ? `${remainingSec}초 후 구사 재경기가 자동 시작됩니다.`
    : `${remainingSec}초 후 자동으로 다음 판이 시작됩니다.`;
...
function renderTurnBanner(state) {
  ...
  if (state.roomStatus === 'lobby') {
    ...
    if (state.pendingReplay) {
      turnBanner.textContent = state.replayInfo?.canJoin
        ? '🔥 구사 재경기 대기 중입니다. 재경기 참가 버튼을 누르면 판돈을 내고 다시 들어갈 수 있습니다.'
        : '🔥 구사 재경기 대기 중입니다. 참가하지 않은 사람은 이번 재경기에서 베팅할 수 없습니다.';
    } else {
...
function renderPlayers(state) {
  ...
  playersList.innerHTML = orderedPlayers
    .map((player) => {
      ...
      return `
        <div class="player-card ${isTurn ? 'turn' : ''} ${isMe ? 'me' : ''} ${player.allIn ? 'all-in-player' : ''} ${state.roomStatus === 'playing' && !player.inRound ? 'not-in-round' : ''} ${player.replayParticipant ? 'replay-participant' : ''} ${player.replayJoinable ? 'replay-joinable' : ''}">
          ...
          <div class="player-badges">
            ...
            ${player.allIn ? '<span class="badge allin">올인</span>' : ''}
            ${state.roomStatus === 'playing' && !player.inRound ? '<span class="badge inactive">재경기 미참가</span>' : ''}
            ${player.replayParticipant ? '<span class="badge replay-in">재경기 참가</span>' : ''}
            ${player.replayJoinable ? '<span class="badge replay-wait">재참가 가능</span>' : ''}
            ${player.handName ? `<span class="badge hand">${escapeHtml(player.handName)}</span>` : ''}
          </div>

          ${player.allIn ? '<div class="player-hand-note allin-note">올인 상태입니다. 더 이상 추가 베팅은 하지 않고 결과만 기다립니다.</div>' : ''}
          ${state.roomStatus === 'playing' && !player.inRound ? '<div class="player-hand-note inactive-note">이번 구사 재경기에 참가하지 않아 베팅할 수 없습니다.</div>' : ''}
...
function renderLastResult(state) {
  ...
  const winnersHtml = result.replay
    ? `<strong>재경기</strong><br />현재 판돈 ${result.carryOverPot || result.pot}조각을 유지합니다.`
    : result.winners
      ...
      return `
        <div class="result-player">
          <div>
            ...
            ${player.allIn ? '<span class="badge allin" style="margin-left:8px;">올인</span>' : ''}
          </div>
  ...
  lastResultBox.innerHTML = `
    <h4>${result.replay ? '재경기 안내' : '지난 판 결과'}</h4>
    <div class="small-label">${escapeHtml(result.reason)}</div>
    <div style="margin-top:8px;"><strong>판돈:</strong> ${result.pot}조각</div>
    <div style="margin-top:8px;"><strong>${result.replay ? '안내' : '승자'}:</strong><br />${winnersHtml || '없음'}</div>
...
function renderReplayPanel(state) {
  if (!replayPanel || !joinReplayBtn || !replayInfoText) return;

  if (!state.pendingReplay || !state.replayInfo) {
    replayPanel.classList.add('hidden');
    joinReplayBtn.disabled = true;
    return;
  }

  const info = state.replayInfo;
  replayPanel.classList.remove('hidden');
  replayInfoText.innerHTML = `
    <strong>구사 재경기 대기 중</strong><br />
    유지 판돈: ${info.carryPot}조각<br />
    재참가 비용: ${info.joinFee}조각<br />
    현재 참가자: ${info.participants.length ? info.participants.map(escapeHtml).join(', ') : '없음'}<br />
    재참가 가능: ${info.joinable.length ? info.joinable.map(escapeHtml).join(', ') : '없음'}
  `;

  joinReplayBtn.disabled = !info.canJoin;
  joinReplayBtn.textContent = info.canJoin
    ? `재경기 참가 (${info.joinFee}조각)`
    : '재경기 참가 불가';
}
...
function renderButtons(state) {
  ...
  [quickAdd10Btn, quickAdd50Btn, quickAdd100Btn, quickAdd500Btn].forEach((button) => {
    if (button) {
      button.disabled = !canAct;
      button.style.opacity = canAct ? '1' : '0.5';
    }
  });
}
...
function addToBetInput(amount) {
  const currentValue = Number(betAmountInput.value || 0);
  const safeCurrent = Number.isFinite(currentValue) ? currentValue : 0;
  betAmountInput.value = String(safeCurrent + amount);
...
joinReplayBtn?.addEventListener('click', () => {
  ensureAudio();
  socket.emit('joinReplay');
});
...
quickAdd10Btn?.addEventListener('click', () => addToBetInput(10));
quickAdd50Btn?.addEventListener('click', () => addToBetInput(50));
quickAdd100Btn?.addEventListener('click', () => addToBetInput(100));
quickAdd500Btn?.addEventListener('click', () => addToBetInput(500));
